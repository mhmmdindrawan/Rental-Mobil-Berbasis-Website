...
header('location:index.php');
	...
	if(isset($_POST['submit'])){
		$status = $_POST['status'];
		$kode = $_POST['id'];
		$mySql	= "UPDATE booking SET status = '$status' WHERE id_booking='$kode'";
		$myQry	= mysqli_query($koneksidb, $mySql);
		$mySql1	= "UPDATE cek_booking SET status = '$status' WHERE id_booking='$kode'";
								...
								<div class="panel panel-default">
									...
									<div class="panel-body">
										...
										$sqlsewa = "SELECT booking.*,mobil.*,merek.*,users.* FROM booking,mobil,merek,users WHERE booking.id_mobil=mobil.id_mobil
													AND merek.id_merek=mobil.id_merek AND users.email=booking.email AND booking.id_booking ='$id'";
